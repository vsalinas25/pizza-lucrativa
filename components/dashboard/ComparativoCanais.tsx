    ...
    <div className="rounded-lg border border-carvao-700 p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <select
          ...
          className="rounded-md bg-carvao-800 border border-carvao-700 px-2.5 py-1.5 text-xs"
        >
          {pizzas.map((p) => (
      ...
      {pizza && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {canais.map((canal) => {
            ...
            return (
              <div key={canal.id} className="rounded-md bg-carvao-800 p-3">
                <p className="text-xs text-trigo-400 mb-1">{canal.nome}</p>
                <p className="font-mono text-sm font-semibold text-trigo-50">
                  {formatarMoeda(preco?.preco_atual ?? 0)}
                </p>
                <p className="font-mono text-xs text-brasa-400">{formatarPercentual(cmv)} CMV</p>
              </div>
            );
          })}
        </div>
